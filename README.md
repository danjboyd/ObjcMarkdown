...
- GitHub Actions runs `linux-appimage` as a thin caller to the reusable `gnustep-packager` workflow pinned to `2903b82e33ae3d219ba3857250be4fe3f798a64d`, using explicit `packager-repository`/`packager-ref` inputs plus this repo's Linux manifest, stage script, and app-specific preflight for the Adwaita theme input. The reusable workflow owns the hosted GNUstep toolchain bootstrap through `gnustep-cli-new`.
