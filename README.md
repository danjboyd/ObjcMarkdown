...
- GitHub Actions runs `linux-appimage` as a thin caller to the reusable `gnustep-packager` workflow pinned to `334c61d4c093531308a20c91559414553eb86946`, using explicit `packager-repository`/`packager-ref` inputs plus this repo's Linux manifest, stage script, and app-specific preflight for the Adwaita theme input. The reusable workflow owns the hosted GNUstep toolchain bootstrap through `gnustep-cli-new`.
- GitHub Actions runs `windows-packaging` as a thin caller to the same pinned reusable workflow and explicit packager checkout inputs, using this repo's Windows MSI manifest and normalized Windows stage script. The reusable workflow owns the hosted MSYS2/GNUstep bootstrap through `gnustep-cli-new`. The staged Windows payload includes the GNUstep runtime, bundled Windows themes, and TinyTeX runtime for external LaTeX rendering. Windows releases are expected to bundle `WinUITheme` and use it as the default packaged theme.
