...
param(
  ...
  [string]$PackagerManifest = "packaging/manifests/windows-msi.manifest.json",
...
throw @"
scripts/windows/oci-run-msi-validation.ps1 is retired.

The supported Windows clean-machine validation path is now OracleTestVMs:
  ./scripts/windows/otvm-msi-validation.sh create --msi <path-to-msi> [--portable-zip <path-to-zip>]

Why:
- the old lower-level OCI helper drifted from the supported Windows bootstrap contract
- OracleTestVMs is now the only supported source of truth for Windows build/test VM provisioning and validation handoff

See:
- docs/windows-otvm-msi-validation.md
- packaging/README.md
- WINDOWS_BUILD.md
"@
