...
Register `ObjcMarkdownViewer` as able to open:
...
Implemented:
- Desktop entry MIME registration includes markdown + importable formats in `Resources/ObjcMarkdownViewer.desktop.in`.
...
Implemented:
- `ObjcMarkdownViewer/ObjcMarkdownViewerInfo.plist` now declares:
...
Prepared:
- `CFBundleDocumentTypes` entries are already present in `ObjcMarkdownViewer/ObjcMarkdownViewerInfo.plist`.
...
- Register a ProgID mapping to `ObjcMarkdownViewer.exe "%1"`.
