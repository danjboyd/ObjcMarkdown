...
## 10) Residual startup negative-width warnings in GNUstep window chrome

- **Status**: Open
- **Opened On**: 2026-03-12
- **Area**: Viewer / GNUstep layout / Theme compatibility
- **Description**: Launching the viewer still emits a small number of `NSView setFrame: given negative width` warnings during initial window setup under GNUstep.
- **Current State**:
  - Explorer sidebar initialization was reordered so its controls are created after the split view has a real sidebar width.
  - That change reduced the warning count from eleven startup warnings to two.
  - The remaining warnings appear during initial window chrome/layout work, not the sidebar control setup path.
- **Next Steps**:
  - Capture one more instrumented launch focused on the remaining two warnings only.
  - Confirm whether the last warnings come from app-managed toolbar/title layout or GNUstep decoration internals.
  - Eliminate any app-owned negative frame path if present; otherwise document the residual GNUstep-only warning as a backend limitation.
